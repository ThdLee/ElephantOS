...
#define PIC_M_CTRL 0x20		// 8259A，主片控制端口是0x20
...
intr_handler idt_table[IDT_DESC_CNT];	// 定义中断处理程序数组。在kernel.S中定义的intrXXentry只是中断处理程序的入口，最终调用的事idt_table中的处理程序
...
static void general_intr_handler(uint8_t vec_nr) {
	...
	if (vec_nr == 14) {	// 若为Pagefault，将缺失的信息打印
		int page_fault_vaddr = 0;
		asm ("movl %%cr2, %0" : "=r" (page_fault_vaddr)); // cr2存放了page_fault的地址
