...
struct arena {
	...
	// large为true时，cnt表示页框数，否则表示空闲mem_block数量
	uint32_t cnt;
	bool large;
};

struct mem_block_desc k_block_descs[DESC_CNT];	// 内核内存块描述符数量
...
void* get_a_page(enum pool_flags pf, uint32_t vaddr) {
	...
	if (cur->pgdir != NULL && pf == PF_USER) {
		...
		PANIC("get_a_pgae: not allow kernel alloc userspace or user alloc kernelspace by get_a_pgae");
	}
...
void* get_a_pgae_without_opvaddrbitmap(enum pool_flags pf, uint32_t vaddr) {
	struct pool* mem_pool = pf & PF_KERNEL ? &kernel_pool : &user_pool;
	lock_acquire(&mem_pool->lock);
	void* page_phyaddr = palloc(mem_pool);
	if (page_phyaddr == NULL) {
...
void* sys_malloc(uint32_t size) {
	...
	if (size > 1024) {
		...
		// 若mem_block_desc的free_list中已经没有可用mem_block，就创建新的arena提供mem_block
		if (list_empty(&descs[desc_idx].free_list)) {
			...
			// 开始讲arena拆分成内存块，并添加到内存块描述符free_list中
...
static void mem_pool_init(uint32_t all_mem) {
	...
	uint32_t used_mem = page_table_size + 0x100000;	// 低端1M内存
	uint32_t free_mem = all_mem - used_mem;
	uint16_t all_free_pages = free_mem / PG_SIZE; // 不足1页的内存不予考虑

	uint16_t half_free_pages = all_free_pages / 2;
	uint16_t kernel_free_pages = half_free_pages - half_free_pages % 8;
	uint16_t user_free_pages = all_free_pages - kernel_free_pages;

	uint32_t kbm_length = kernel_free_pages / 8;	// kernel BitMap的长度
	uint32_t ubm_length = user_free_pages / 8;		// User BitMao的长度
	...
	user_pool.pool_size = (user_free_pages - user_free_pages % 8) * PG_SIZE;
