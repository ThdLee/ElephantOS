...
static void partition_format(struct partition* part) {
	...
	uint32_t inode_bitmap_sects = DIV_ROUND_UP(MAX_FILES_PER_PART, BITS_PER_SECTOR);	// i结点位图占用的扇区数，最多占用4096个文件
	...
	// 初始化块位图block_bitmap
	buf[0] |= 0x01;		// 第0个块留给根目录，位图中先占位
	uint32_t block_bitmap_last_byte = block_bitmap_bit_len / 8;
	uint8_t block_bitmap_last_bit = block_bitmap_bit_len % 8;
	uint32_t last_size = SECTOR_SIZE - (block_bitmap_last_byte % SECTOR_SIZE);	// last_size是位图所在最后一个扇区中，不足一个扇区的其余部分

	// 2.1 先将位图最后一字节到其所在的扇区结束全置为1，即超出实际块数的部分直接标为已占用
	...
	/* 由于inode_table中共4096个inode，位图inode_bitmap正好占用1个扇区
	 * 即inode_bitmap_sects等于1，所以位图中的位全部代表inode_table中的inode，
	 * 无须再像blcok_bitmap那样单独处理最后一扇区的剩余部分，
	 * inode_bitmap所在的扇区中没有多余的无效位 */
	ide_write(hd, sb.inode_bitmap_lba, buf, sb.inode_bitmap_sects);

	// 4 将inode数组初始化并写入sb.inode_table_lba
	// 准备些inode_table中的第0项，即根目录所在的inode
...
static int search_file(const char* pathname, struct path_search_record* searched_record) {
	...
	while (name[0]) {
		...
		if (search_dir_entry(cur_part, parent_dir, name, &dir_e)) {
			memset(name, 0, MAX_FILE_NAME_LEN);
			// 若sub_path不等于NULL，也就是未结束，继续拆分路径
			if (sub_path) {
				sub_path = path_parse(sub_path, name);
			}
			...
			// 若找不到，则返回-1
			// 找不到目录项时，不要关闭parent_dir，可能会在parent_dir中创建新文件
			return -1;
		}
	}

	// 执行到此，必然是遍历了完整路径并查找的文件或目录只有同名目录存在
...
int32_t sys_open(const char* pathname, uint8_t flags) {
	...
	if (searched_record.file_type == FT_DIRECTORY) {
		printk("cant't open a directory with open(), use opendir() to install\n");
		dir_close(searched_record.parent_dir);
		return -1;
	}
	...
	//先判断是否把pathname的各目录访问到了，即是否在某个中间目录就失败了
	if (pathname_depth != path_searched_depth) {	// 说明并没有访问到全部路径
		printk("cannot access %s: Not a directory, subpath %s doesn't exist\n", pathname, searched_record.searched_path);
		dir_close(searched_record.parent_dir);
		return -1;
	}
	...
	if (!found && !(flags & O_CREAT)) {
		printk("in path %s, file %s doesn't exist\n", searched_record.searched_path, (strrchr(searched_record.searched_path, '/') + 1));
		dir_close(searched_record.parent_dir);
		return -1;
	...
	// 此fd是指任务pcb->fd_table数组中的元素下标，并不是指全局file_table下标
	return fd;
}
...
int32_t sys_lseek(int32_t fd, int32_t offset, uint8_t whence) {
	...
	switch (whence) {
		...
		case SEEK_SET:
			new_pos = offset;
			break;
		// SEEK_CUR 新的读写位置是相对于文件尺寸再增加offset个位移量
		case SEEK_CUR:
...
int32_t sys_unlink(const char* pathname) {
	...
	if (searched_record.file_type == FT_DIRECTORY) {
		printk("con't delete a directory with unlink(), use rmdir() to instead\n");
		dir_close(searched_record.parent_dir);
		return -1;
	}
...
int32_t sys_mkdir(const char* pathname) {
	...
	if (inode_no != -1) {	// 如果找到了同名目录或文件，失败返回
		...
		if (pathname_depth != path_searched_depth) {	// 说明并没有访问到全部的路径，某个中间目录是不存在的
			printk("sys_mkdir: can't access %s, subpath %s doesn't exist\n", pathname, searched_record.searched_path);
			rollback_step = 1;
			goto rollback;
		}
	}
...
struct dir* sys_opendir(const char* name) {
	...
	if (inode_no == -1) {	// 如果找不到目录，提示不存在的路径
		printk("in %s, subpath %s doesn't exist\n", name, searched_record.searched_path);
	} else {
...
// 读取目录dir的1个目录项，成功后返回其目录项地址，到目录尾时或出错是返回NULL
...
static int get_child_dir_name(uint32_t p_inode_nr, uint32_t c_inode_nr, char* path, void* io_buf) {
	...
	if (parent_dir_inode->i_sectors[12]) {	// 若包含着一级间接块表，将读入all_blocks
	...
	while (channel_no < channel_cnt) {
		...
		while (dev_no < 2) {
			...
			while (part_idx < 12) {	// 4个主分区+8个逻辑
				 ...
				 * partition又为disk的嵌套结构，依次partition中的成员默认也为0
