...
// 在part分区内的pdir目录内寻找名为name的文件或目录，找到后返回true并将其目录存入dir_e
bool search_dir_entry(struct partition* part, struct dir* pdir, const char* name, struct dir_entry* dir_e) {
	...
	if (pdir->inode->i_sectors[12] != 0) {	// 若含有一级间接表
		ide_read(part->my_disk, pdir->inode->i_sectors[12], all_blocks+12, 1);
	}
	// 至此，all_blocks存储的是该文件或目录的所有扇区地址

	// 写目录项的时候已经保证目录项不跨扇区，这样读目录项时容易处理，只申请容纳1个扇区的内存
...
bool sync_dir_entry(struct dir* parent_dir, struct dir_entry* p_de, void* io_buf) {
	...
	// 将该目录的所有扇区地址（140个块）存入all_blocks
	uint8_t block_idx = 0;
	...
	// 开始遍历所有块以寻找目录项空位，若已有扇区中没有空闲为，在不超过文件大小的情况下申请新扇区来存储新目录项
	block_idx = 0;
	while (block_idx < 140) {
		block_bitmap_idx = -1;
		if (all_blocks[block_idx] == 0) {
		...
		while (dir_entry_idx < dir_entrys_per_sec) {
			if ((dir_e + dir_entry_idx)->f_type == FT_UNKNOWN) {	// FT_UNKNOWN为0，无论是初始化或是删除文件后，都会讲f_tyoe置为FT_UNKNOWN
...
bool delete_dir_entry(struct partition* part, struct dir* pdir, uint32_t inode_no, void* io_buf) {
	...
	while (block_idx < 140) {
			...
			if ((dir_e + dir_entry_idx)->f_type != FT_UNKNOWN) {
				if (!strcmp((dir_e + dir_entry_idx)->filename, ".")) {
					...
					if ((dir_e + dir_entry_idx)->i_no == inode_no) {	// 如果找到此i结点，就将其记录在dir_entry_found
						ASSERT(dir_entry_found == NULL);	// 确保目录中只有一个编号为inode_no的inode
		...
		// 除目录第一个扇区外，若该扇区上只有该目录项，则将整个扇区回收
	...
	// 所有块中未找到则返回false，若出现这种清空应该是search_file出错了
	return false;
}
...
struct dir_entry* dir_read(struct dir* dir) {
	...
	uint32_t cur_dir_entry_pos = 0;	// 当前目录项的偏移，此项用来判断是否是之前已返回过的目录项
	uint32_t dir_entry_size = cur_part->sb->dir_entry_size;
	...
	while (block_idx < block_cnt) {
		...
		// 遍历扇区内所有目录项
		while (dir_entry_idx < dir_entrys_per_sec) {
			if ((dir_e + dir_entry_idx)->f_type) { // 判断f_type不为FT_UNKNOWN
...
int32_t dir_remove(struct dir* parent_dir, struct dir* child_dir) {
	...
	if (io_buf == NULL) {
		printk("dir_remove: molloc for io_buf failed\n");
		return -1;
	}
