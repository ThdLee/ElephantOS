...
// 从文件表file_table中获取一个空闲为，成功返回下标，失败返回-1
...
// 将全局描述符下标安装到进程或线程自己文件描述符数组fd_table中，成功返回下标，失败返回-1
int32_t pcb_fd_install(int32_t global_fd_idx) {
	...
	if (local_fd_idx == MAX_FILES_OPEN_PER_PROC) {
		printk("exceed max open file files_per_proc\n");
		return -1;
	}
	return local_fd_idx;
}
...
void bitmap_sync(struct partition* part, uint32_t bit_idx, uint8_t btmp_type) {
	uint32_t off_sec = bit_idx / MAX_FILES_PER_PART;	// 本i结点索引相对于位图的扇区偏移量
	uint32_t off_size = off_sec * BLOCK_SIZE;	// 本i结点索引相对与位图的字节偏移量
	uint32_t sec_lba;
	uint8_t* bitmap_off;

	// 需要被同步到硬盘的位图只有inode_bitmap和blcok_bitmap
	switch (btmp_type) {
		case INODE_BITMAP:
...
// 打开编号为inode_no的inode对应的文件，若成功则返回文件描述符，负责返回-1
...
int32_t file_write(struct file* file, const void* buf, uint32_t count) {
	...
	// 写入count个字节前，该文件已经占用的块数 DIV_ROUND_UP
	uint32_t file_has_used_blocks = file->fd_inode->i_size / BLOCK_SIZE + 1;

	// 存储count字节后该文件将占用的块数 DIV_ROUND_UP
...
// 从文件file中读取count个字节写入部分， 返回读出的字节数，若到文件结尾则返回-1
int32_t file_read(struct file* file, void* buf, uint32_t count) {
	uint8_t* buf_dst = (uint8_t*)buf;
	uint32_t size = count, size_left = size;

	// 若要读取的字节数超过了文件可读取的剩余量，就用剩余量作为待读取的字节数
	...
	uint32_t* all_blocks = (uint32_t*)sys_malloc(BLOCK_SIZE + 48);	// 用来记录文件所有块的地址
	if (all_blocks == NULL) {
