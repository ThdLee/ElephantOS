...
// 用于记录总扩展分区的起始lba，初始为0，partition_scan时以此位标记
...
static void select_sector(struct disk* hd, uint32_t lba, uint8_t sec_cnt) {
	...
	/* 因为lba地址24~27位要存储在device寄存器的0~3位，
...
static void cmd_out(struct ide_channel* channel, uint8_t cmd) {
	// 只要想硬盘发送了命令便将此标记置位true，硬盘中断处理程序需要根据它来判断
...
static void read_from_sector(struct disk* hd, void* buf, uint8_t sec_cnt) {
	...
	if (sec_cnt == 0) {
		// 因为sec_cnt是8位变量，由主调函数将其赋值时，若位256则会将最高位的1丢掉变为0
...
void ide_read(struct disk* hd, uint32_t lba, void* buf, uint32_t sec_cnt) {
	...
	while (secs_done < sec_cnt) {
		...
		/* 在硬盘已经开始工作（开始在内部读取或写数据）后才能阻塞自己，现在硬盘已经开始忙了，
		...
		if (!busy_wait(hd)) {	// 若失败
			char error[64];
			sprintf(error, "%s read secotr %d failed!!!\n", hd->name, lba);
			PANIC(error);
		}
...
void ide_write(struct disk* hd, uint32_t lba, void* buf, uint32_t sec_cnt) {
	...
	while (secs_done < sec_cnt) {
		...
		if (!busy_wait(hd)) {	// 若失败
			char error[64];
			sprintf(error, "%s write secotr %d failed!!!\n", hd->name, lba);
			PANIC(error);
		}
...
static bool partition_info(struct list_elem* pelem, int arg UNUSED) {
	...
	// 此处return只是为了让主调函数list_traversal继续向下遍历元素
	return false;
}
...
void ide_init() {
	...
	channel_cnt = DIV_ROUND_UP(hd_cnt, 2);	// 根据硬盘数反推ide通道
	...
	while (channel_no < channel_cnt) {
		...
		switch (channel_no) {
			case 0:
				...
				channel->irq_no = 0x20 + 14;	// 从片8259a上倒数第二个中断引脚，硬盘，也就是ide0通道的中断向量号
				break;
			case 1:
				channel->port_base = 0x170;		// ide1通道的起始端口号是0x170
				channel->irq_no = 0x20 + 15;	// 从片上最后一个中断引脚，用来响应ide1通道上的硬盘中断
				break;
		}

		channel->expecting_intr = false;		// 未向硬盘写入指令时不期待硬盘中断
		lock_init(&channel->lock);

		/* 初始化为0，目的是想硬盘控制器请求数据后，硬盘驱动sema_down此信号会阻塞线程，
