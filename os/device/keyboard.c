...
static void intr_keyboard_handler(void) {
	...
	if (break_code) {	// 若断码是break_code(按键弹起时产生的扫描码)
		...
		return;
	} // 若为通码，值处理数组中定义的键以及alt_right和ctrl键，全是make_code
	else if ((scancode > 0x00 && scancode < 0x3b) || \
		(scancode == alt_r_make) || (scancode == ctrl_r_make)) {
		bool shift = false;	// 判断是否与shift组合，用来在一维数组中索引对应字符
		if ((scancode < 0x0e) || (scancode == 0x29) || \
	    	...
	    	*******************************/
			...
			if (shift_down_last && caps_lock_last) {
				shift = false;
			} else if (shift_down_last || ctrl_down_last) {
				shift = true;
			} else {
				shift = false;
			}
		...
		uint8_t index = (scancode &= 0x00ff);	// 将扫描码高字节置0，主要针对高字节是e0的扫描码
