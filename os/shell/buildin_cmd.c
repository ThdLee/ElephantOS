...
static void wash_path(char* old_abs_path, char* new_abs_path) {
	...
	if (name[0] == 0) {	// 若值键入了"/"，直接将"/"存入new_abs_path后返回
		new_abs_path[0] = '/';
		new_abs_path[1] = 0;
		return;
	}
	...
	while (name[0]) {
		...
		if (!strcmp("..", name)) {
			...
			// 如果未到new_abs_path中的顶层目录，就将最右边的'/'替换为0，这样便去除了new_abs_path中最后一层路径
			if (slash_ptr != new_abs_path) {
				*slash_ptr = 0;
			} else {	// 如果new_abs_path为"/a"，".."之后为"/"
				// 若new_abs_path中只有1个'/'，即表示已经到了顶层目录，就将下一个字符置为0
				*(slash_ptr + 1) = 0;
			}
		} else if (strcmp(".", name)) {	// 如果路径不是'.'，就将name拼接到new_abs_path
			if (strcmp(new_abs_path, "/")) { // 如果new_abs_path不是"/"，就拼接一个"/"
				strcat(new_abs_path, "/");
			}
...
void make_clear_abs_path(char* path, char* final_path) {
	...
	if (path[0] != '/') {	// 若输入的不是绝对路径，就拼接成绝对路径
		...
		if (getcwd(abs_path, MAX_PATH_LEN) != NULL) {
			if (!((abs_path[0] == '/') && (abs_path[1] == 0))) {	// 若abs_path表示的目录不是根目录
				strcat(abs_path, "/");
			}
		}
	}
		...
		if (long_info) {
			...
			while ((dir_e = readdir(dir))) {
				...
				if (stat(sub_pathname, &file_stat) == -1) {
					printf("ls: cannot acess %s: No such file or directory\n", dir_e->filename);
					return;
				}
